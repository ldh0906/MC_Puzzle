...
Paper 1.20.1 / Java 17용 1–4인 파티 미궁 플러그인입니다. 로비에서 `/maze`를 사용하면 파티 전용 void 차원이 만들어지고, `[미궁] A to Z v8.scx`에서 분석한 소재를 마인크래프트에 맞게 재설계한 20개 방을 순서대로 풉니다.

50개 방 대표 5개 MVP는 폐기되었습니다. 현재 공식 팩은 `a-to-z-archive-20` 하나이며, 발판·버튼·레버를 정답 입력에 사용하지 않습니다.

## 현재 구현

- 1–4인 파티 생성, 초대, 수락/거절, 추방, 해산
- 파티별 `mcpuzzle_<UUID>` 전용 void 월드와 20개 방 자동 생성
- 방마다 서로 다른 블록 바닥 도식과 자동 지급되는 증거 기록서
- `/maze answer <정답>` 텍스트 입력, 유니코드/대소문자 정규화
- 오답 뒤 10–18초 파티 공용 입력 잠금으로 무작위 대입 억제
- 방 단위 체크포인트, 3개 세이브 슬롯, 정확한 원래 명단만 재개
...
방별 해법 구조와 공정성 기준은 [20개 방 설계표](map-packs/a-to-z-archive-20/ROOM_DESIGN.md)에 정리되어 있습니다.

## 빌드와 검증

```powershell
python map-packs/a-to-z-archive-20/generate_map.py
python map-packs/a-to-z-archive-20/validate_pack.py
...
- 플러그인이 `2.1.0-a20`의 20개 방을 로드해 READY가 되는가
- 서버 데이터 폴더에 최신 공식 맵이 배포되는가
- 임시 인스턴스에 20개 방·바닥 도식·표지판을 실제 생성하고 금지 입력 블록이 없는가
...
| `/maze leaderboard [1-4]` | 파티 크기별 순위표 |
...
활성 팩: `plugins/MCPuzzle/map-packs/a-to-z-archive-20/map.jsonc`
...
lobby:
  world: world
  spawn: {x: -43.5, y: 97.0, z: 0.5, yaw: 0.0, pitch: 0.0}
resource-pack:
  required: true
...
- `map-packs/a-to-z-archive-20`: 공식 20방 생성기, 검증기, 설계표
